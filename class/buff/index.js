...
class buff_class {
  constructor({ cookie, baseURL = 'https://buff.163.com/api' } = {}) {
    this.config = {
      baseURL,
      headers: {
  ...
  async getBalance() {
    return this.request('/asset/get_brief_asset', 'GET');
  }
  ...
  async balance()
  {
    try{
        ...
        return Number(bal.data?.cash_amount)/7; //Value in USD
    }catch(e)
    {
        console.error(e)
        return 0;
