...
class igxe_class {
  constructor({ cookie, baseURL = 'https://www.igxe.cn' } = {}) {
    this.config = {
      baseURL,
      headers: {
  ...
  async getBalance() {
    return this.request('/user/get_user_top_info', 'GET');
  }
  ...
  async balance()
  {
    try{
        ...
        return Number(bal.cash_amount)/7; //Value in USD
    }catch(e)
    {
        console.error(e)
        return 0;
