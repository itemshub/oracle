...
const requestInjuection = async (page)=>
{
  ...
  page.on('response', async (response) => {
    try {
      ...
      if(url.includes("api/market/goods/sell_order"))
      {
        //The maker api
      }
      if(url.includes("api/market/goods/buy_order"))
      {
        //The taker api
      }
...
//urls
const makerUrl = "https://buff.163.com/goods/34989?from=market#tab=selling"
const takerUrl = "https://buff.163.com/goods/34989?from=market#tab=buying"

async function fetchData() {
    const batchId = Date.now();
    await runWithTimeout(() => getData(0,makerUrl), 120000, 0);
    await sleep(120000);
}
