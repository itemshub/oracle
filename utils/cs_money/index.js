...
const requestInjuection = async (page)=>
{
  ...
  page.on('response', async (response) => {
    try {
      ...
      if(url.includes("market/sell-orders"))
      {
        // console.log(body)
        //The maker api
...
{
    maker_data = {};
    await fetchData(`https://cs.money/2.0/market/sell-orders?limit=60&offset=0&name=${cases.name}`)
    // console.log(maker_data)
    if(maker_data?.items && maker_data.items?.length > 0)
    {
      return maker_data.items[0]?.pricing ? maker_data.items[0].pricing.computed : 0
    }
   return 0 ;
}
